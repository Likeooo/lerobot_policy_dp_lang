        ...
        with h5py.File(file, "r") as f:
            for timestamp in f["data"].keys():
                ...
                for i in range(images.shape[0]):
                    ...
                    dataset.add_frame(
                        {
                            "observation.image_0": images[i][0], #right0
                            "observation.image_1": images[i][1], #right1
                            "observation.image_2": images[i][2], #left0
                            "observation.image_3": images[i][3], #left1
                            "observation.image_4": images[i][4], #forward
                            "observation.image_wrist": images[i][5], #wrist 
                            "observation.state": ee_state[i],
                            "action": action,
                            "task": np.array(f["data"][timestamp]["instruction"])[0].decode("utf-8"),
                        }
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset-path", type=str, default="/home/lhs/dataset/vlabench", help="Path to the dataset")
...
# python scripts/convert_to_lerobot.py  --task-list set_study_table --dataset-name bevdp/select_fruit_apple