...
# --- 路径配置 ---
TASK_NAME = "get_coffee_random_pos_fix_asset"
MODEL_TYPE = "dp_lang" # 
MODEL_NAME = f"get_coffee_random_pos_fix_asset_6camera_gpu_8_bz_8"
ROOT_PATH=f"/meta_eon_cfs/home/ljh/.cache/huggingface/lerobot/vlabench/{TASK_NAME}"
PORT = "13024"
CAMERA_PERTURBATION = "large" # fix small medium large
SAVE_DIR = f"eval_{CAMERA_PERTURBATION}_cam"
# SAVE_DIR = "debug_zero_state"  # 调试模式下的保存目录


# 项目路径配置 (请确保这些是文件夹的绝对路径)
SERVER_PROJECT_PATH = "/meta_eon_cfs/home/ljh/code/lerobot_policy_dp_lang"
CLIENT_PROJECT_PATH = "/meta_eon_cfs/home/ljh/code/BEV_BENCH"
CKPT_ROOT = f"{SERVER_PROJECT_PATH}/outputs/{MODEL_TYPE}/{MODEL_NAME}/checkpoints"
LOG_FILE = f"{SERVER_PROJECT_PATH}/outputs/{MODEL_TYPE}/{MODEL_NAME}/{SAVE_DIR}_evaluated_checkpoints.txt"
POLL_INTERVAL = 600
...
CLIENT_PYTHON = "/meta_eon_cfs/home/ljh/.conda/envs/bev_bench/bin/python"

# --- 显存与负载配置 ---
MIN_FREE_MEMORY = 3500  # 单位 MiB
...
def run_evaluation(ckpt_path, ckpt_name):
    ...
    # 已经在bashrc设置egl相关环境变量，暂时不在这里设置
    
    # 启动 Server
    server_cmd = [
        SERVER_PYTHON, f"{SERVER_PROJECT_PATH}/src/web_evaluate/server.py",
        f"--dataset.repo_id=vlabench/{TASK_NAME}",
        # f"--dataset.repo_id=xxx",
    ...
    # 启动 Client
    client_cmd = [
        CLIENT_PYTHON, f"{CLIENT_PROJECT_PATH}/scripts/evaluate_multiviewvla.py",
        "--tasks", TASK_NAME,
        "--n-episode", "50",
        "--visulization",
        "--save-dir", SAVE_DIR,
        "--port", PORT,
        "--camera-perturbation", CAMERA_PERTURBATION
    ]
